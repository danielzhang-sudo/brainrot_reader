    ...
    try:
        with zipfile.ZipFile(book_path, 'r') as container:
            ...
            text_files = [
                ...
                if f.lower().endswith(('.xhtml', '.html', '.htm', '.xml')) and not "toc" in f.lower()
            ]
            # Maintain a consistent alphabetical/structural sort sequence
            text_files.sort()
            return text_files
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed parsing ePub archive structure: {str(e)}")

@app.post("/api/v1/library/upload")
async def upload_to_library(file: UploadFile = File(...)):
    if not file.filename.endswith(".epub"):
        raise HTTPException(status_code=400, detail="Only standard .epub extensions accepted.")
    
    target_path = os.path.join(STORAGE_DIR, file.filename)
    with open(target_path, "wb") as destination:
        shutil.copyfileobj(file.file, destination)
    
    return {"message": "Successfully archived into backend storage system.", "filename": file.filename}
...
async def list_book_chapters(book: str = Query(...)):
    ...
    chapters_metadata = [
        {"index": idx, "id": filepath, "title": os.path.basename(filepath).replace(".xhtml","").replace(".html","").replace("_"," ")}
...
async def stream_chapter(book: str = Query(...), chapter_index: int = Query(...)):
    ...
    if chapter_index < 0 or chapter_index >= len(sections):
        raise HTTPException(status_code=400, detail="Target chapter index boundary overflow.")
